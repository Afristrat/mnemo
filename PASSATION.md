...
[ETAT]    prd.json = **60 stories, 50 faites, 10 restantes** (ÉPIC RÉSIDENCE/DR S-043→S-048 + ÉPIC i18n S-057→S-060). **ÉPIC LLM COMPLET** (S-034→040 + 052 + 053). **S-053 ACTIVÉ EN PROD** (admin live, cf. ci-dessous). origin/main = `379dd9e`. Qualité : typecheck 0 · lint 0/0 · **356 unit + 10 skip** · build OK. ✅ **PROD = MAIN** : redéployée 2026-05-28T~14:18 (deploy `playxlhhkise7alzrgh1turd`, finished) et **VÉRIFIÉE LIVE** : assistant `/api/llm/chat` → `ok=True`, 3 sources web, cite **exactement** le chiffre fourni « 137 €/mois (±30 %) » sans en inventer (garde-fou DÉFCON 1 par construction OK). Proxy LiteLLM `proxy.ai-mpower.com` (200), `deepseek-v4-flash`.
[ENCOURS] **RAS — checkpoint propre, prod à jour.** Aucune story à mi-chemin. **✅ S-053 PLEINEMENT ACTIF EN PROD** : migration `20260528120000_admin_prompts.sql` appliquée au Supabase self-hosté ; compte super-admin `medamine.mansouriidrissi@gmail.com` (id `f056b97e-0e14-497c-94b7-1c2d11beba7a`) créé via GoTrue admin + seedé dans `super_admins` (count=1) ; 3 vars Supabase posées dans Coolify (`NEXT_PUBLIC_SUPABASE_URL`+`ANON_KEY` buildtime, `SUPABASE_SERVICE_ROLE_KEY` runtime) ; redéployé ; **login vérifié (grant password OK) + rôle super-admin vérifié (RLS self-select renvoie la ligne)**. Console éditable sur https://infra.ai-mpower.com/admin (e-mail + mdp du coffre `MNEMO_ADMIN_*`).
...
[ALERTE]  ⚠ **Déploiement** : utiliser **`COOLIFY_PUBLIC_URL`** (PAS `COOLIFY_URL` = IP LAN injoignable) ; PS 5.1 → forcer TLS 1.2. · **Supabase prod = self-hosté** (SSH `serveurai`, conteneur `supabase-db`, compose `/home/serveurai/stacks/supabase/docker`) ; SQL via base64→`docker exec -i supabase-db psql` ; body avec secret via **scp fichier** (PAS stdin PowerShell = encodage casse le JSON). Tables prod = init_rails + prompts/super_admins (S-053) ; conversion(S-023)/catalog_obs(S-036) NON appliquées (aucun consommateur prod). · **LLM tokens à l'usage NON chiffré** (dette → story future). · **Prix de vente = [PLACEHOLDER]**. · **e2e local fragile** (`next start` KO en `output:standalone`) → viser prod via `E2E_BASE_URL=https://infra.ai-mpower.com`. · Coolify env var champ `is_buildtime`. · webhook GitHub→Coolify non configuré (redeploy manuel API).
