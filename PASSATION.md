...
```
== PASSATION STRATE 2026-05-27T13:10 ==
[ETAT]    Lot 1 ✅ + REFONTE 10/10 ✅ + **Homepage S-032 ✅ + S-025 price feed LIVE ✅ + S-026 backup moteur ✅ + S-033 compute souverain (fondation) ✅ + Renommage Strate ✅ + tirets « — »→« , » ✅**. **TOUT DÉPLOYÉ EN PROD** (infra.ai-mpower.com sert « Strate », `/api/pricing/live` 8/8). origin/main = `5f3f258` (tree propre hors `wtp-research.md` untracked + `*.png` gitignorés). unit **204+8skip** · typecheck 0 · lint 0/0 · build OK · RLS LIVE 8/8 · e2e 11+1skip. ⚠ S-026 backup & S-033 compute = **moteurs purs faits + testés mais PAS encore intégrés dans `recommend`** → backup & serveurs **pas encore VISIBLES** dans l'UI/coûts.
[ENCOURS] **PROCHAINE = intégration groupée dans `recommend`** (rend backup + compute VISIBLES = attendu d'Amine « je ne vois pas les serveurs »). Élargir `recommend(profile, prices, computePrices?)` : appeler `buildBackupPlan` + `computeSovereignCompute`, **remplacer le forfait C6 hardcodé** (`layers.ts` 30/100/400) par le compute dimensionné (laisser une part LLM estimée + note), imputer backup aux couches/setup, remplir `Recommendation.{backup, compute}`. **Change la signature** → threader `ensemble.ts` + `ResultsView` (injecter le live via `/api/pricing/live` étendu compute) + ajuster les tests de coût. Coupler avec **S-027** (9ᵉ dim `resilience`).
[FAIT]    Session 2026-05-27 (suite) : **S-032 Homepage** déployée. **S-025 price feed LIVE** (firecrawl.scrapeStructuredJson + reconcile garde-fou + live-feed + /api/pricing/live + LivePriceStatus), validé LIVE 8/8, déployé, ADR-019. **S-026 backup moteur** (types, deriveBackupPlan+costBackup purs, rétention légale SOURCÉE, tier cold additif, arbitrage vecteurs ; 19 tests). **S-033 compute fondation** (compute.ts + compute-seed Scaleway sourcé + doc ; 7 tests). **Renommage Mnémo→Strate** (code + docs vivants, déployé). **Tirets « — »→« , »** dans l'UI + exports (fix fin-de-ligne JSX inclus, ex. lead hero), déployé. **Portée décidée par Amine : soin tirets/français = UNIQUEMENT visible + partageable + export MD/PDF** (pas le code interne).
[ALERTE]  ⚠ **Prix de VENTE = `[PLACEHOLDER]`** (sondage). · **favicon.ico 404** (à ajouter, global). · **LLM tokens** sortis du forfait C6 = **estimation à chiffrer** (story future) — ne pas oublier en intégrant le compute. · Périmètre live = GPU/stockage/backup/compute **€ Scaleway** ; API tierces $ = seed+FX (inextractibles). · `MediaNeedsBlock` (wizard) = seed sync (aperçu) ; RÉSULTATS = live. · **Présenter, ne pas orienter**. · `mnemo` technique CONSERVÉ (package/repo/Coolify/domaine/localStorage) — renommer = opération à part (casse déploiement). · Seul `—` visible volontaire = `<option>—</option>` du sondage (placeholder « aucune sélection » ; Amine doit trancher : garder ou « Choisir… »). · Supabase serveur non exposé (:8200). · webhook GitHub→Coolify non configuré (redeploy manuel API).
[BLOQUE]  RAS
[NEXT]    **CE QUI RESTE** — ① **Intégration `recommend` (backup S-028 + compute) + S-027 resilience 9ᵉ dim** → backup & serveurs VISIBLES (CostMap/verdict/radar 9 axes ; `toHaveLength(8)`→9). ② **S-029** Exit Escrow piloté par le plan · **S-030** UI Bloc ② sauvegarde (RadioCards criticité + expert) · **S-031** résultats backup + e2e + nettoyage. ③ **Coût LLM tokens live** (story neuve, prix tokens Mistral/Claude). ④ **EPIC i18n COMPLET FR/AR/EN/ES** (décidée Amine) : next-intl, extraction de toutes les chaînes, **RTL arabe**, traductions ; touche l'UI **ET** le texte produit par le moteur (`verdict`/`reason`/`pain` etc.) → prévoir d'externaliser ces chaînes hors du moteur. ⑤ **favicon** · **placeholder sondage `—`** (décision) · **Spec n°2** multi-juridiction/résidence + DR (brainstorm→spec→plan) · **sondage→prix de vente** (remplacer `[PLACEHOLDER]`). ⑥ Optionnel : aligner `MediaNeedsBlock` (wizard) sur le live ; renommer les identifiants techniques `mnemo`.
[MEMO]    Coffre secrets GLOBAL ~/.claude/secrets/secrets.env.dpapi (load-secrets.ps1). Redeploy prod : `POST $COOLIFY_URL/api/v1/deploy?uuid=$MNEMO_APP_UUID&force=true` (Bearer `$COOLIFY_API_TOKEN`). Moteurs prêts à intégrer : `buildBackupPlan(profile,sizing,hotStorage,backupPrices)` + `computeSovereignCompute(profile,preset,computePrices)` + `getBackupPrices()`/`getComputePrices()`. Mémoires CLÉS : `prix-jamais-hardcodes-feed-live` · `definition-de-termine` · `strate-avis-critique-non-oriente`. « reprends en Ralph » relit `.ralph/prd.json`+`progress.md`+AGENTS.md AVANT d'agir.
```
