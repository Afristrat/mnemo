...
```
== PASSATION STRATE 2026-05-27T11:35 ==
[ETAT]    Lot 1 ✅ + **REFONTE Strate 10/10 ✅ + Homepage de vente (S-032) ✅ + S-025 price feed LIVE ✅**. **REFONTE + HOMEPAGE DÉPLOYÉES EN PROD** (infra.ai-mpower.com sert désormais la refonte — PLUS le Lot 1). ⚠ **S-025 PAS encore en prod** (commit `0a17539` poussé, redeploy non fait). origin/main = `0a17539` (tree propre hors `*.png` gitignorés + `wtp-research.md` untracked). unit **178+8skip** · typecheck 0 · lint 0/0 · build OK · RLS LIVE **8/8** · e2e 11+1skip. Backup : spec+plan faits (S-026→S-031 `passes=false`).
[ENCOURS] **PROCHAINE = S-026 — `lib/engine/backup.ts`** (deriveBackupPlan + costBackup **purs**, prix injectés, spec §3/§4/§7). Le **prérequis S-025 est FAIT** : `getLivePrices()` (médias **+ backup**) et `getBackupPrices()`/`BACKUP_PRICE_SEED` (egress/archive/retrait/hors-site sourcés) prêts à consommer dans le moteur.
[FAIT]    Session 2026-05-27 (suite) : **S-032 Homepage de vente** (`app/page.tsx` ← portage `homepage-draft.html` : 4 promesses, wedge, étapes, tarifs `[PLACEHOLDER]`, diagnostic 90 s) **+ refonte déployées en prod** (redeploy Coolify). **S-025 Price feed LIVE Firecrawl** : faisabilité testée (H100 0 € sur page d'agrégat → garde-fou obligatoire), `lib/pricing/{firecrawl.scrapeStructuredJson, reconcile, backup-seed, live-feed}`, route `/api/pricing/live`, `LivePriceStatus`, `ResultsView` injecte le live dans `recommend`. **Validé LIVE 8/8 postes** (H100 dédié 1992,9 €/mois, pas 0). ADR-019.
[ALERTE]  ⚠ **Prix de VENTE = `[PLACEHOLDER]`** (sondage en cours, mémoire `definition-de-termine`). · **favicon.ico 404** (aucun favicon — global, à ajouter). · Périmètre live S-025 = GPU/stockage/backup **€ Scaleway** ; **API tierces $ (OpenAI/Google/Runway) restent seed+FX** — inextractibles fiablement, ne PAS « promouvoir » l'inextractible (DÉFCON 1). · `MediaNeedsBlock` (wizard) utilise encore le seed sync pour l'aperçu ; les **RÉSULTATS** utilisent le live. · **Présenter, ne pas orienter** (`strate-avis-critique-non-oriente`). · Renommage Mnémo→Strate **non fait** (UI dit « Mnémo »). · Supabase serveur non exposé (localhost:8200). · webhook GitHub→Coolify non configuré (redeploy manuel via API).
[BLOQUE]  RAS
[NEXT]    ① **redéployer la prod** pour y inclure S-025 (`/api/pricing/live`) si voulu. ② **Backup S-026→S-031** : backup.ts (plan+coût+arbitrage vecteurs) · **9ᵉ dim `resilience`** (refonte 8→9, transverse — inventorier les `toHaveLength(8)`) · intégration recommend · Exit Escrow piloté par le plan · UI Bloc ② · résultats+e2e. ③ **Spec n°2** multi-juridiction/résidence + DR (brainstorm→spec→plan). ④ Renommage Strate · sondage→prix de vente.
[MEMO]    Coffre secrets GLOBAL ~/.claude/secrets/secrets.env.dpapi (load-secrets.ps1). Redeploy prod : `POST $COOLIFY_URL/api/v1/deploy?uuid=$MNEMO_APP_UUID&force=true` (Bearer `$COOLIFY_API_TOKEN`). Mémoires CLÉS : `prix-jamais-hardcodes-feed-live` (S-025 l'implémente) · `definition-de-termine` · `strate-avis-critique-non-oriente` · `strate-backlog-usecases-manquants`. « reprends en Ralph » relit `.ralph/prd.json`+`progress.md`+AGENTS.md AVANT d'agir.
```
