...
```
== PASSATION STRATE 2026-05-27T09:45 ==
[ETAT]    Lot 1 ✅ + déployé prod (infra.ai-mpower.com — c'est encore le Lot 1, PAS la refonte). **REFONTE Strate 10/10 ✅ (S-015→S-024)** poussée. origin/main = `ff13d7c` (à jour, tree propre hors `.firecrawl/`+`wtp-research.md`). unit **163+8skip** · typecheck 0 · lint 0/0 · build OK · RLS LIVE **8/8** · **e2e 11+1skip**. **Backup : spec + plan FAITS** (S-025→S-031 ajoutées au prd, `passes=false`).
[ENCOURS] **PROCHAINE = S-025 reprofilée → « Price feed LIVE Firecrawl »** (PRÉREQUIS au backup). ⚠ Amine a **rejeté les prix hardcodés** : le but = extraction structurée LIVE des prix vendor (Firecrawl JSON/LLM, runtime, serveur), seed = **repli daté + baseline** seulement. **À reprendre AUSSI le pricing médias S-017** (aujourd'hui `getMediaPrices()` renvoie des littéraux). Cf. mémoire `prix-jamais-hardcodes-feed-live`. (Le code backup hardcodé a été supprimé ; baseline sourcée conservée dans `docs/pricing/backup-cost-sources.md`.)
[FAIT]    Session 2026-05-27 : refonte S-016→S-024 (sizing GPU 1× · pricing médias sourcé + FX BCE · intégration verdict/setup · wizard 4 blocs · bloc Médias + budget-mètre · chemin 90 s · résultats verdict + CostMap · conversion/data RLS 8/8 · e2e). **Spec+plan backup** (`…specs/2026-05-27-strate-backup-design.md`, `…plans/2026-05-27-strate-backup.md`). **UX** : budget-mètre à DROITE sticky + configurateur pleine largeur (`max-w-[1400px]`). Design poussé. ADR-010→018.
[ALERTE]  ⚠⚠ **PRIX JAMAIS HARDCODÉS** — feed Firecrawl LIVE obligatoire, seed = repli (mémoire `prix-jamais-hardcodes-feed-live`). Le média actuel viole ça → à corriger en S-025. · **« Terminé » ≠ prd coché** : fini = déployé + cohérent (nom) + vrais use-cases (mémoire `definition-de-termine`). · **Présenter, ne pas orienter** ; l'utilisateur tranche (`strate-avis-critique-non-oriente`). · Prix de VENTE = `[PLACEHOLDER]` (sondage). · Renommage Mnémo→Strate **non fait** (UI dit « Mnémo »). · Refonte **non déployée**. · Routes HTTP simulation/lead + UI capture exit-intent non faites (mécanisme S-023 prêt). · Supabase serveur non exposé (localhost:8200). · webhook GitHub→Coolify non configuré (redeploy manuel).
[BLOQUE]  RAS
[NEXT]    ① **S-025 — Price feed LIVE Firecrawl** (média + backup ; extraction structurée ; seed=repli+baseline ; DÉFCON 1 : valider live vs baseline, source+fraîcheur, repli) — PRÉREQUIS. ② **Backup S-026→S-031** (backup.ts plan+coût+arbitrage vecteurs · 9ᵉ dim `resilience` · intégration recommend · Exit Escrow · UI Bloc ② · résultats+e2e). ③ **Spec n°2** multi-juridiction/résidence + DR (brainstorm→spec→plan). ④ Renommage Strate · déployer la refonte · backlog `docs/BACKLOG-USECASES.md` · sondage→prix→homepage.
[MEMO]    Coffre secrets GLOBAL ~/.claude/secrets/secrets.env.dpapi (load-secrets.ps1). Mémoires CLÉS ajoutées cette session : `prix-jamais-hardcodes-feed-live` · `definition-de-termine` · `strate-avis-critique-non-oriente` · `strate-backlog-usecases-manquants`. « reprends en Ralph » relit `.ralph/prd.json`+`progress.md`+AGENTS.md AVANT d'agir.
```
