...
- **Problème** : les prix d'infra étaient sourcés mais **figés dans le seed** (littéraux). Objectif F4 / mémoire `prix-jamais-hardcodes-feed-live` : valeurs **extraites en direct** des pages vendor, le seed n'étant que repli + baseline.
- **Faisabilité testée AVANT de coder (DÉFCON 1)** : l'extraction structurée Firecrawl (`format json` + schéma/`prompt` LLM) est **fiable sur une page dédiée** (page H100 → 2,73 €/h) mais **renvoie des valeurs aberrantes sur une page d'agrégat** (`/pricing/gpu/` → H100 à **0 €**, prix chargé dynamiquement). Conclusion : on ne peut JAMAIS injecter le live aveuglément.
- **Décision** : extraction live **primaire**, mais chaque valeur passe un **garde-fou** (`reconcilePrice`, pur) qui la confronte à la baseline sourcée : promue (`live`) si dans la bande ±60 %, sinon **rejetée au profit de la baseline** (`flagged`, « à revérifier », confiance basse), ou repli daté (`fallback`) si extraction indisponible. La baseline seed est donc à la fois repli réseau ET filtre de plausibilité.
- **Architecture** : `lib/pricing/firecrawl.scrapeStructuredJson` (extraction), `lib/pricing/reconcile` (garde-fou pur), `lib/pricing/backup-seed` (prix egress/archive/retrait/hors-site, sourcés `docs/pricing/backup-cost-sources.md`), `lib/pricing/live-feed` (`getLivePrices` : scrape GPU+H100+stockage Scaleway 1× chacun, €/h GPU → mensuel ×heures, réconcilie médias **et** backup, cache 6 h). Route serveur `/api/pricing/live` (clé jamais exposée). `ResultsView` rend d'abord le seed (repli immédiat) puis bascule sur le live injecté dans `recommend`. Composant `LivePriceStatus` (provenance : statut + source + fraîcheur).
- **Périmètre live** : GPU souverain + stockage + postes backup (€ Scaleway, extraction nette). Les **API tierces à l'usage** ($ OpenAI/Google/Runway, unités dérivées) restent au seed sourcé + étage FX — non extractibles de façon fiable (promouvoir l'inextractible = injecter du faux).
- **Conséquence** : tests 178 + 8 skip (reconcile 7, backup-seed 3, live-feed 5 dont H100=0 rejeté, ×13 rejeté, repli total). typecheck 0, lint 0/0, build OK. **Validation LIVE réelle** : `/api/pricing/live` → 8/8 postes extraits et promus, H100 à 1992,9 €/mois (page dédiée) et non 0. Reste pour les stories backup (S-026+) : consommer `getBackupPrices`/`getLivePrices().backup` dans le moteur.

## ADR-020 — Preset scoré + explicable, réconciliation étiquette↔règles (S-051)

- **Problème** (remonté par Amine) : les presets semblaient incohérents. Diagnostic confirmé : `decidePreset` était une **cascade de règles écrites à la main** (portée de la maquette v2, non dérivée de données), et surtout le champ `expected` des `PRESET_PROFILES` était une **étiquette d'auteur DIVERGENTE du résultat des règles** — ex. « Coach indépendant » étiqueté `MEDIUM` mais classé `LIGHT` par la cascade (sensibilité confidentielle + solo + petit budget/volume). Divergence jamais réconciliée (cf. learning S-004 « ne jamais tester contre `expected` »).
- **Décision** (Amine, option « refonte scorée **2 + 1** ») : modèle en **deux temps, explicable**.
  1. **Déclencheurs durs → HARD** (souveraineté/conformité prime, indépendamment de la charge) : `secret` · `cabinet-regule` · `hipaa` · `secret-pro` · (`audit` ET `bitemporal`). Inchangé (défendable).
  2. Sinon, un **score de besoin** départage LIGHT/MEDIUM. Le **volume domine** (séparateur principal : `< 1 Go` = base « personnelle » → 0 pt ; `1–10 Go` = +4 → premier corpus d'organisation), complété par sensibilité (public 0 / confidentiel 1 / interne 2), débit, utilisateurs, voix, croissance, budget et nature de l'activité. **Seuil LIGHT = score ≤ 3** (calibré : score max d'un cas LIGHT-cible = 3 ; min d'un cas MEDIUM = 4).
- **Réconciliation** : le seuil et les poids sont **calibrés pour que les 6 profils-types calculent EXACTEMENT leur `expected`** (Coach → MEDIUM, DIY → LIGHT, etc.). Test de garde dans `engine.test` : `decidePreset(p.profile).preset === p.expected` pour chacun → **zéro divergence** (le caveat S-004 est levé : on PEUT désormais tester contre `expected`).
- **Explicabilité** : `PresetDecision` expose `{ preset, reason, score, drivers }` ; `reason` cite le score, le seuil et les contributions dominantes (« Besoin intermédiaire (score 7 > 3) : volume 10–100 Go (+6), interne (+2)… ») → affiché via `Recommendation.presetReason`. Preset « expliqué », pas une boîte noire.
- **Calibrage assumé** : les poids sont des **hypothèses de modélisation** (comme les facteurs ±30 % du coût), pas des vérités absolues — documentées ici, ajustables. La charge (volume/débit/users) pilote AUSSI le dimensionnement (compute/sizing) ; ici elle ne sert qu'à choisir le **palier d'architecture**.
- **Conséquence** : `preset.ts` réécrit (déclencheurs + score pur). Fixture `light` de `bundle.test` rendue réellement minuscule (volume < 1 Go, solo, public) — l'ancienne (5 users, multi-voix, croissance forte) est désormais MEDIUM à juste titre. Tests : +`preset.test.ts` (10 : déclencheurs durs, séparateur volume, monotonie du score, explicabilité) + réconciliation dans `engine.test`. Tous les cas `decidePreset` historiques (HARD/LIGHT/MEDIUM) restent verts sans modification. typecheck 0, lint 0/0, build OK.

## ADR-021 — Composition de l'Infra Health Score (IHS) : pondération + seuils (S-079, F12)

- **Problème** : le monitoring (Lot 2 · C) a besoin d'un **score de santé composite** de l'infra déployée, mais un score composite est un terrain glissant pour la fiabilité (DÉFCON 1) — des poids et seuils « tirés du chapeau » donneraient une fausse impression de précision.
- **Décision** : `lib/monitoring/health.ts` — `computeHealthScore(input)` **pur, signaux injectés** (zéro I/O). Six dimensions, chacune un sous-score normalisé **0–100** : `availability` (disponibilité services), `resilience` (sauvegarde/DR en place et testées), `compliance` (actions de conformité traitées), `budget` (adhérence coût réel vs cible), `freshness` (fraîcheur des prix/veilles sourcés), `drift` (conformité config déployée ↔ reco).
- **Pondération (échelle ordinale ASSUMÉE, révisable)** : `availability=3`, `resilience=2`, `compliance=2`, `budget=1`, `freshness=1`, `drift=1`. Justification : une base **inaccessible** (availability) ou des **données perdues** (resilience) rendent tout le reste vain → poids vital ; un risque **juridique** (compliance) est grave → poids élevé ; budget/fraîcheur/dérive **dégradent la qualité** sans être vitaux → poids unitaire. Ce ne sont pas des vérités absolues mais une hiérarchie défendable et documentée (comme les facteurs ±30 % du coût, ADR-020).
- **Seuils de statut** : `ok ≥ 80`, `warn 50–79`, `critical < 50` (sur 0–100). Appliqués au composite ET à chaque dimension.
- **Garde-fou DÉFCON 1 — « on ne fabrique pas un score qu'on n'a pas »** : une dimension **non mesurée** (signal absent) est **EXCLUE du composite** (jamais comptée comme zéro, ce qui pénaliserait à tort), avec statut `unknown` ; le composite est la moyenne pondérée des **seules dimensions mesurées** (et `null` si rien n'est mesuré). Le rapport expose `measured/total` → l'UI peut signaler la couverture réelle de la mesure. Les **alertes** = dimensions mesurées en `warn`/`critical` (descripteurs i18n).
- **i18n** : aucun libellé en dur — sorties = descripteurs `Message` (`Engine.monitoring.health.*`), résolus en fr/en/ar à la présentation (S-081).
- **Conséquence** : `computeHealthScore` est le contrat ; la **dérivation des signaux depuis les vraies métriques** d'exploitation (uptime…) et depuis la reco/coût réel se branche en S-081/S-082. Tests : monotonie (un signal mesuré qui baisse → composite ≤), exclusion des non-mesurés, seuils, alertes.

## ADR-022 — Dispatch Deviation Guide : règles d'exploitation dégradée façon MEL (S-080, F12)

- **Problème** : un score de santé (ADR-021) dit *qu'*un problème existe, pas *quoi faire*. Il faut une conduite à tenir par dégradation — mais « inventer » des procédures au fil de l'eau serait contraire à DÉFCON 1.
- **Décision** : `lib/monitoring/mel.ts` — `deriveDeviationGuide(report)` **pur**, dérivé du `HealthReport`. Analogie **MEL (Minimum Equipment List)** de l'aviation : pour chaque dimension **dégradée** (warn/critical), un item = `{ dispatch, guidance, corrective }`. `dispatch` ∈ `go-if` (exploitation permise sous condition/surveillance) | `no-go` (mode protégé : lecture seule / gel des écritures / suspension du traitement concerné).
- **Règle de dispatch (documentée, non ad-hoc)** : un `critical` sur **availability** (base inaccessible), **resilience** (risque de perte de données) ou **compliance** (non-conformité avérée) ⇒ `no-go` ; les autres dimensions (budget/freshness/drift) dégradent la qualité sans interdire l'exploitation ⇒ `go-if`. Tout `warn` ⇒ `go-if`. Le catalogue de conduites (guidance) + correctifs est porté en i18n (`Engine.monitoring.mel.*`), une entrée par dimension.
- **Garde-fou** : seules les dimensions **mesurées et dégradées** produisent un item — aucune conduite n'est émise sur une dimension non observée. Ordre = celui des sous-scores (priorité de pondération ADR-021 préservée).
- **Conséquence** : le moteur produit le guide ; l'affichage (dashboard) et la persistance arrivent en S-081. Tests : dispatch no-go/go-if par dimension×statut, items seulement sur dégradé, descripteurs i18n.
